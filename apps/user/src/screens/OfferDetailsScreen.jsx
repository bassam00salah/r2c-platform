  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="p-6 pb-24">
        ...
        <div className="flex items-center gap-2 text-gray-500 mb-6"><span>📍</span><span>{selectedOffer.city}{selectedOffer.distance ? ` • ${selectedOffer.distance}` : ''}</span></div>
        <div className="bg-gradient-to-r from-[#ee7b26]/20 to-[#d96b1a]/20 border border-[#ee7b26]/30 rounded-2xl p-6 mb-6">
          ...
          <div className="space-y-2 mb-6">{(selectedOffer.details || []).map((detail, index) => <div key={index} className="flex items-center gap-2"><span className="text-[#ee7b26]">•</span><span>{detail}</span></div>)}</div>
          <div className="flex items-baseline gap-3">{price !== null && price !== undefined ? <span className="text-[#ee7b26] text-5xl font-bold">{Number(price).toLocaleString('ar-SA')} ريال</span> : <span className="text-gray-400 text-lg">السعر غير محدد</span>}{oldPrice !== null && oldPrice !== undefined && <span className="text-gray-500 text-xl line-through">{Number(oldPrice).toLocaleString('ar-SA')} ريال</span>}</div>
        </div>
        <div className="flex items-center justify-center gap-2 bg-amber-50 border border-amber-200 rounded-xl py-3 mb-6"><span className="text-[#ee7b26]">⏱</span><span className="font-semibold text-amber-800">صالح لمدة {offerDuration} دقيقة بعد قبول الطلب</span></div>
